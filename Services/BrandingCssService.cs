        ...
        public static string BuildCss(PluginConfiguration config)
        {
            ...
            if (hasLogo)
            {
                ...
                if (config.ReplaceHeaderLogo)
                {
                    ...
                    // .pageTitleWithDefaultLogo is given its background-image by the active
                    // theme stylesheet, so the override has to be !important to win regardless
                    // of stylesheet order. Class names verified identical in 10.11.11 and 12.0.
